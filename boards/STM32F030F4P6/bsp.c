...
bool bsp_uart_tx_ready(void) {
    return (BSP_UART->ISR & USART_ISR_TXE) != 0u;
}

bool bsp_uart_rx_ready(void) {
    return (BSP_UART->ISR & USART_ISR_RXNE) != 0u;
}

void bsp_uart_putc(char character) {
    while (!bsp_uart_tx_ready()) {
    }
    BSP_UART->TDR = (uint8_t)character;
}
...
char bsp_uart_getc(void) {
    ...
    return (char)(BSP_UART->RDR & 0xFFu);
}
...
bool bsp_uart_try_getc(char* character) {
    ...
    *character = (char)(BSP_UART->RDR & 0xFFu);
    return true;
}

void bsp_uart_flush(void) {
    while ((BSP_UART->ISR & USART_ISR_TC) == 0u) {
    }
}
