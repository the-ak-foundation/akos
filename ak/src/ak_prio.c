...
#define AK_NPRIO_PER_entry      (sizeof(size_t) << 3)
#define AK_NPRIO_PER_entry_POW2 CTZ(AK_NPRIO_PER_entry)
#define AK_PRIO_BITSET_size \
    (((AK_PRIO_MAX + AK_NPRIO_PER_entry - 1) >> AK_NPRIO_PER_entry_POW2) + 1)

static size_t _ak_prio_bitset[AK_PRIO_BITSET_size];
...
int ak_prio_bit_set(ak_prio_t prio) {
    ...
    size_t entry = prio >> AK_NPRIO_PER_entry_POW2;
    size_t offset = prio & (AK_NPRIO_PER_entry - 1);
    _ak_prio_bitset[entry] |= 1LL << offset;
    return 0;
}
...
ak_prio_t ak_prio_get_top(void) {
    size_t res = (AK_PRIO_BITSET_size << AK_NPRIO_PER_entry_POW2) - 1;
    for (int i = AK_PRIO_BITSET_size - 1; i >= 0; --i) {
        if (_ak_prio_bitset[i]) {
            res -= CLZ(_ak_prio_bitset[i]);
            break;
        }
        res -= AK_NPRIO_PER_entry;
    }
    return res;
}
