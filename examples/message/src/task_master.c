...
void task_master(void *p_arg) {
    ...
    timer = akos_timer_create(0u, MSG_SIGNAL_TIMER, NULL, THREAD_MASTER_ID,
    ...
    while (true) {
        ...
        if (message != NULL) {
            ...
            case MSG_SIGNAL_TIMER: {
                ...
                PRINT_DBG(
                    "[%02lu-%02lu-%02lu %03lu ms] [%s]\tsend \"%s\" to "
                    "task Slave\r\n",
                    (unsigned long)TIME_HOURS(ticks),
                    (unsigned long)TIME_MINUTES(ticks),
                    (unsigned long)TIME_SECONDS(ticks),
                    (unsigned long)TIME_MILLIS(ticks), NOTE, MESSAGE_TEXT);

                akos_thread_post_msg_dynamic(
                    THREAD_SLAVE_ID, MSG_SIGNAL_TEXT, MESSAGE_TEXT,
