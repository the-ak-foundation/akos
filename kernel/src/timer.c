...
static ak_timer_t timer_pool[AKOS_CFG_TIMER_POOL_SIZE];   /**< Static storage for all timer objects. */
static ak_timer_t *free_list_timer_pool;                /**< Head of free timer-object list. */
static uint8_t timer_pool_used;                         /**< Number of allocated timer objects. */

static list_t timer_list_1;                             /**< Active/overflow timer list storage. */
static list_t timer_list_2;                             /**< Active/overflow timer list storage. */

static list_t *volatile timer_list_ptr;                 /**< Timer list for the current tick range. */
static list_t *volatile overflow_timer_list_ptr;        /**< Timer list for trigger ticks after wraparound. */

static volatile uint32_t next_tick_to_unblock_timer \
                        = (uint32_t)AKOS_CFG_DELAY_MAX;   /**< Next timer trigger tick. */
...
static void timer_pool_init()
{
    ...
    for (index = 0; index < AKOS_CFG_TIMER_POOL_SIZE; index++)
    {
        if (index == (AKOS_CFG_TIMER_POOL_SIZE - 1))
        {
...
static void timer_switch_lists()
{
    ...
    {
        next_tick_to_unblock_timer = AKOS_CFG_DELAY_MAX;
    }
    else
    {
...
ak_timer_t *akos_timer_create(timer_id_t id, int32_t sig, timer_cb func_cb, uint8_t des_thread_id, uint32_t period, timer_type_t type)
{
    ...
    ak_timer_t *p_timer;
    AKOS_CORE_ENTER_CRITICAL();
    if ((timer_pool_used >= AKOS_CFG_TIMER_POOL_SIZE) || (free_list_timer_pool == NULL))
    {
        AKOS_CORE_EXIT_CRITICAL();
