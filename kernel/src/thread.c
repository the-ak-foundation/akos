...
/** @brief Priority used by the internal idle task. */
#define TASK_IDLE_PRI               (AKOS_CFG_PRIO_MAX - 1u)

/** @brief Priority used by the internal timer task. */
#define TASK_TIMER_PRI              ((uint8_t)AKOS_CFG_TIMER_TASK_PRI)

/** @brief Stack size in words for the internal timer task. */
#define TASK_TIMER_STK_SIZE         (100u)

/** @brief Internal task control block type. */
typedef struct task_tcb task_tcb_t;

/** @brief Size of one task control block. */
#define SIZE_OF_TCB                 (sizeof(task_tcb_t))


task_tcb_t *volatile tcb_curr_ptr = NULL;           /**< Currently running task control block. */
task_tcb_t *volatile tcb_high_rdy_ptr = NULL;       /**< Highest-priority ready task control block. */

static list_t rdy_task_list[AKOS_CFG_PRIO_MAX];       /**< Ready task lists indexed by priority. */
static list_t dly_task_list_1;                      /**< Active/overflow delayed task list storage. */
static list_t dly_task_list_2;                      /**< Active/overflow delayed task list storage. */
static list_t *volatile dly_task_list_ptr;          /**< Delayed task list for the current tick range. */
static list_t *volatile overflow_dly_task_list_ptr; /**< Delayed task list for wake ticks after wraparound. */
static list_t suspended_task_list;                  /**< Tasks that are currently suspended. */

static volatile uint16_t num_of_tasks           = (uint16_t)0U;                 /**< Number of created tasks. */
static volatile uint32_t tick_count             = (uint32_t)0u;                 /**< System tick counter. */
static volatile uint32_t ticks_pended           = (uint32_t)0U;                 /**< Ticks pended while scheduler is locked. */
static volatile uint32_t next_tick_to_unblock   = (uint32_t)AKOS_CFG_DELAY_MAX;   /**< Next wake tick for delayed tasks. */

static volatile uint8_t sched_is_running        = (uint8_t)OS_FALSE;            /**< Scheduler running flag. */

extern const thread_t __start_task_desc[]       __attribute__((weak));          /**< First linked static thread descriptor. */
extern const thread_t __stop_task_desc[]        __attribute__((weak));          /**< One-past-last linked static thread descriptor. */

static task_tcb_t **task_tcb_list = NULL;       /**< Runtime TCB table indexed by thread ID. */
static uint8_t task_tcb_list_len = 0u;          /**< Number of entries in the runtime TCB table. */
static uint8_t task_app_count = 0u;             /**< Number of application thread descriptors. */
static uint8_t task_shell_id = UINT8_MAX;       /**< Runtime ID assigned to shell task. */
static uint8_t task_idle_id = 0u;               /**< Runtime ID assigned to idle task. */
static uint8_t task_timer_id = 0u;              /**< Runtime ID assigned to timer task. */
...
/**
 * @brief Get number of statically registered application threads.
...
/**
 * @brief Get runtime ID assigned to the idle task.
 * @return Idle task ID.
 */
...
/**
 * @brief Get runtime ID assigned to the shell task.
 * @return Shell task ID, or UINT8_MAX when shell is disabled.
 */
...
/**
 * @brief Get runtime ID assigned to the timer task.
 * @return Timer task ID.
 */
...
/**
 * @struct task_tcb
 * @brief Runtime task control block used by the scheduler.
 */
struct task_tcb
{
    volatile uint32_t *stk_ptr;   /**< Saved stack pointer; must be first for port context switch. */
    list_item_t state_list_item;  /**< Node used by ready, delayed, and suspended lists. */
    list_item_t event_list_item;  /**< Node used by event/message wait lists. */
    uint32_t *stk_limit_ptr;      /**< Lowest valid stack address used as stack limit. */
    uint8_t prio;                 /**< Scheduler priority; lower value means higher priority. */
    size_t stk_size;              /**< Task stack size in 32-bit words. */
    uint32_t *stk_base_ptr;       /**< Base address of task stack. */
    thread_id_t id;               /**< Runtime thread ID. */
    msg_queue_t msg_queue;        /**< Per-thread message queue. */
    thread_state_t state;         /**< Current scheduler state. */
    uint32_t run_ticks;           /**< Accumulated running ticks. */
    uint32_t last_run_ticks;      /**< Last runtime accounting sample. */
    uint8_t  cpu_load;            /**< Cached CPU load percentage. */
};

#if (AKOS_CFG_USE_SHELL != 0u)
/**
 * @brief Look up display name for a runtime thread ID.
 * @param id Runtime thread ID.
 * @return Thread name, or NULL when no name is available.
 */
...
#if (AKOS_CFG_USE_RUNTIME_STATS != 0u)
/**
 * @brief Update runtime counters during a context-switch decision.
 */
...
static void init_task_lists(void)
{
    /* Initialize lists */
    uint8_t prio;
    for (prio = 0; prio < AKOS_CFG_PRIO_MAX; prio++)
    {
...
static void task_switch_delay_lists()
{
    ...
    if (list_is_empty(dly_task_list_ptr) == OS_TRUE)
    {
        ...
        next_tick_to_unblock = AKOS_CFG_DELAY_MAX;
    }
    else
    {
...
static void add_curr_task_to_delay_list(uint32_t tick_to_delay, uint8_t can_block_indefinitely)
{
    ...
    if ((tick_to_delay == AKOS_CFG_DELAY_MAX) && (can_block_indefinitely != OS_FALSE))
    {
 ...
 * @param id Task ID.
 * @param pf_thread Task entry function.
                               ...
                               size_t stack_size)
{
    ...
    if (stack_size < AKOS_CFG_TASK_STK_SIZE_MIN)
    {
    ...
    //( void ) memset( p_new_tcb->stk_limit_ptr, AKOS_CFG_TASK_STACK_FILL_BYTE, stack_size );
...
void akos_thread_register_static_threads(void)
{
    ...
    (void)task_create((thread_id_t)task_shell_id,
                      ...
                      (void *)NULL,
                      (uint8_t)AKOS_CFG_SHELL_TASK_PRI,
                      (size_t)AKOS_CFG_SHELL_TASK_MSG_Q_SIZE,
                      (size_t)AKOS_CFG_SHELL_TASK_STK_SIZE);
#endif
    ...
    (void)task_create((thread_id_t)task_timer_id,
                      ...
                      (size_t)(AKOS_CFG_TASK_MSG_Q_SIZE_NORMAL),
    ...
    (void)task_create((thread_id_t)task_idle_id,
                      ...
                      (size_t)(0u),
                      (size_t)AKOS_CFG_TASK_STK_SIZE_MIN);
}

#if (AKOS_CFG_USE_RUNTIME_STATS != 0u)
/**
 * @brief Update runtime tick accounting for current and next running task.
 */
static void akos_thread_calculate_cpu_load(void)
{
...
/**
 * @brief Read accumulated idle and total runtime counters.
 * @param p_idle_ticks Output idle runtime in ticks.
 * @param p_total_ticks Output total runtime in ticks.
 */
...
/**
 * @brief Get number of runtime thread table entries.
 * @return Number of runtime thread slots.
 */
...
/**
 * @brief Copy runtime information for one thread table entry.
 * @param index Thread table index to snapshot.
 * @param p_snapshot Output snapshot buffer.
 */
 ...
 * @param time_out Timeout in ticks. Use @ref AKOS_CFG_DELAY_MAX for infinite wait.
...
msg_t *akos_thread_wait_for_msg(uint32_t time_out)
{
    ...
    if (time_out > (uint32_t)0U && p_msg == NULL)
    {
        ...
        if (time_out == AKOS_CFG_DELAY_MAX)
        {
