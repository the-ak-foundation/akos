...
struct task_tcb {
    ...
    list_item_t state_list_item; /**< Item in ready, blocked, or suspended state
                                    list. */
...
static list_t dly_task_list_2;                      /**< Delayed tasks that have overflowed the
                                                       current tick count. */
static list_t* volatile dly_task_list_ptr;          /**< Points to the delayed task list
                                                       currently being used. */
static list_t* volatile overflow_dly_task_list_ptr; /**< Points to the delayed
                                                       task list currently being
                                                       used to hold overflowed
                                                       tasks. */
...
static volatile uint32_t next_tick_to_unblock =
    (uint32_t)OS_CFG_DELAY_MAX; /**< Initialised to portMAX_DELAY before the
                                   scheduler starts. */
...
static void add_curr_task_to_delay_list(uint32_t tick_to_delay, uint8_t can_block_indefinitely) {
    ...
    if ((tick_to_delay == OS_CFG_DELAY_MAX) && (can_block_indefinitely != OS_FALSE)) {
        ...
        if (time_to_wake < const_tick) {
            ...
            /*Update next tick to block is important in order scheduler not to
            miss this stamp Just update next tick to block in this branch
            because overflow delay is just the background list
            */
 ...
 */
static task_tcb_t* task_create(thread_id_t id, thread_func_t pf_thread, void* p_arg, uint8_t prio,
                               size_t queue_size, size_t stack_size) {
    ...
    //( void ) memset( p_new_tcb->stk_limit_ptr, OS_CFG_TASK_STACK_FILL_BYTE,
    // stack_size );

    /*Init stack frame*/
    p_stack_ptr = akos_port_task_stack_init(p_stack, stack_size, pf_thread, p_arg);
...
void akos_thread_register_static_threads(void) {
    ...
    if ((p_thread_desc == NULL) || (p_thread_desc_end == NULL) ||
        (p_thread_desc == p_thread_desc_end)) {
    ...
    for (; p_thread_desc < p_thread_desc_end; ++p_thread_desc) {
        ...
        (void)task_create(p_thread_desc->id, p_thread_desc->pf_thread, p_thread_desc->p_arg,
                          p_thread_desc->prio, p_thread_desc->queue_size,
                          p_thread_desc->stack_size);
    }

    (void)task_create((thread_id_t)task_timer_id, (thread_func_t)task_timer_func, (void*)NULL,
                      (uint8_t)TASK_TIMER_PRI, (size_t)(OS_CFG_TASK_MSG_Q_SIZE_NORMAL),
                      (size_t)TASK_TIMER_STK_SIZE);

    (void)task_create((thread_id_t)task_idle_id, (thread_func_t)task_idle_func, (void*)NULL,
                      (uint8_t)TASK_IDLE_PRI, (size_t)(0u), (size_t)OS_CFG_TASK_STK_SIZE_MIN);
}
...
void akos_thread_post_msg_dynamic(uint8_t des_thread_id, int32_t sig, void* p_content,
                                  uint8_t msg_size) {
    AKOS_CORE_ENTER_CRITICAL();
    if ((task_tcb_list == NULL) || (des_thread_id >= task_tcb_list_len) ||
        (task_tcb_list[des_thread_id] == NULL)) {
    ...
    case THREAD_STATE_SUSPENDED_ON_MSG:
        akos_message_queue_put_dynamic(&(task_tcb_list[des_thread_id]->msg_queue), sig, p_content,
                                       msg_size);
 ...
 * @param time_out Timeout in ticks. Use @ref OS_CFG_DELAY_MAX for infinite
 * wait.
...
msg_t* akos_thread_wait_for_msg(uint32_t time_out) {
    ...
    if (time_out > (uint32_t)0U && p_msg == NULL) {
        add_curr_task_to_delay_list(time_out, OS_TRUE);
