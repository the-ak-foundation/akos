 ...
 *
 * @author  JiaHui | Snoopy3921 - AK Foundation
...
static void memory_heap_init(void) {
    ...
    mem_blk_end_ptr           = (mem_blk_header_t*)mem_heap;
...
void* akos_memory_malloc(size_t size) {
    ...
    if ((p_block->size - size) > (MIN_SIZE_TO_SPLIT + SIZE_OF_BLOCK_HEADER)) {
        mem_blk_header_t* p_new_block =
            (mem_blk_header_t*)(((uint8_t*)p_block) + SIZE_OF_BLOCK_HEADER + size);
...
void akos_memory_free(void* p_addr) {
    ...
    uint8_t is_above_heap_end =
        ((uint8_t*)p_addr >=
         ((uint8_t*)mem_blk_end_ptr + SIZE_OF_BLOCK_HEADER + mem_blk_end_ptr->size));
