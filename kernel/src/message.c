...
static msg_t msg_pool[AKOS_CFG_MSG_POOL_SIZE];
...
static void msg_pool_init(void)
{
    ...
    for (index = 0; index < AKOS_CFG_MSG_POOL_SIZE; index++)
    {
        if (index == (AKOS_CFG_MSG_POOL_SIZE - 1))
        {
                              ...
                              uint8_t size)
{
    ...
    if (msg_pool_used >= AKOS_CFG_MSG_POOL_SIZE)
    {
