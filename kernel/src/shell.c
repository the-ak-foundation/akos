 ...
 * @date	2026/05/08
 * @module	AKOS
 * @brief	Shell command interface implementation.
 */
...
static uint8_t          shell_rx_buffer[AKOS_CFG_SHELL_RX_BUFFER_SIZE];
static volatile bool    shell_rx_notified;

static char             shell_line_buffer[AKOS_CFG_SHELL_LINE_BUFFER_SIZE];
...
/**
 * @brief Initialize shell RX buffer and diagnostic state.
 */
...
void akos_shell_thread(void *p_arg)
{
    ...
    for (;;)
    {
        msg_t *msg = akos_thread_wait_for_msg(shell_top_active ? AKOS_SHELL_TOP_REFRESH_TICKS : AKOS_CFG_DELAY_MAX);
        if (msg == NULL)
        {
            if (shell_top_active)
            {
...
/**
 * @brief Queue one received byte for shell processing.
 * @param byte Received byte from the shell transport.
 */
void akos_shell_rx_isr(uint8_t byte)
{
    const uint8_t shell_thread_id = akos_thread_get_shell_thread_id();
    const uint8_t next = (uint8_t)((shell_rx_head + 1u) % AKOS_CFG_SHELL_RX_BUFFER_SIZE);
...
/**
 * @brief Drain queued RX bytes and pass them to the line editor.
 */
static void shell_process_rx(void)
{
    for (;;)
    {
        ...
        shell_rx_tail = (uint8_t)((shell_rx_tail + 1u) % AKOS_CFG_SHELL_RX_BUFFER_SIZE);
        shell_process_byte(byte);
    }
}

/**
 * @brief Process one byte of shell input.
...
static void shell_process_byte(uint8_t byte)
{
    ...
    if (shell_line_length < (AKOS_CFG_SHELL_LINE_BUFFER_SIZE - 1u))
    {
...
/**
 * @brief Parse and execute the current shell command line.
 */
static void shell_execute_line(void)
{
    char *argv[AKOS_CFG_SHELL_MAX_ARGS];
    int argc;
    int consumed_argc;
    ...
    argc = shell_tokenize(shell_line_buffer, argv, (int)AKOS_CFG_SHELL_MAX_ARGS);
    if (argc <= 0)
    {
        return;
    }
...
/**
 * @brief Split a command line into whitespace-separated arguments.
 * @param line Mutable command line buffer.
 * @param argv Output argument vector.
 * @param argv_max Maximum number of arguments to store.
...
/**
 * @brief Print built-in and registered shell commands.
 */
static void shell_show_help(void)
{
    const akos_shell_cmd_t *cmd = __start_shell_cmd_desc;
...
/**
 * @brief Render the live runtime load screen.
 */
static void shell_show_top(void)
{
    ...
    for (prio = 0u; prio < AKOS_CFG_PRIO_MAX; ++prio)
    {
        for (index = 0u; index < runtime_limit; ++index)
        {
            thread_runtime_snapshot_t snapshot;
...
/**
 * @brief Enter live runtime load screen mode.
 */
static void shell_top_enter(void)
{
...
/**
 * @brief Leave live runtime load screen mode and restore the prompt.
 */
static void shell_top_exit(void)
{
    shell_top_active = false;
...
/**
 * @brief Print the shell prompt string.
 */
static void shell_prompt(void)
{
    SYS_PRINT(AKOS_CFG_SHELL_PROMPT_NAME);
}

/**
 * @brief Match a registered command path against parsed command arguments.
 * @param path Registered command path.
 * @param argc Number of parsed arguments.
 * @param argv Parsed argument vector.
 * @param consumed_argc Output number of arguments consumed by the path.
...
/**
 * @brief Shell feature initialization stub when shell support is disabled.
 */
void akos_shell_feature_init(void)
{
}
...
/**
 * @brief Shell thread stub when shell support is disabled.
 * @param p_arg Unused thread argument.
 */
void akos_shell_thread(void *p_arg)
{
...
/**
 * @brief Shell RX stub when shell support is disabled.
 * @param byte Ignored received byte.
 */
void akos_shell_rx_isr(uint8_t byte)
{
    (void)byte;
