...
/* Defines -------------------------------------------------------------------*/
#define list_item_set_owner(p_list_item, p_owner)      ((p_list_item)->owner_ptr = (void *)(p_owner))       /**< Set owner pointer of list item. */
#define list_item_get_owner(p_list_item)               ((p_list_item)->owner_ptr)                           /**< Get owner pointer of list item. */
#define list_item_set_value(p_list_item, val)          ((p_list_item)->value = (uint32_t)(val))             /**< Set ordered-list sort value. */
#define list_item_get_value(p_list_item)               ((p_list_item)->value)                               /**< Get ordered-list sort value. */
#define list_item_get_list_contain(p_list_item)        ((p_list_item)->list_ptr)                            /**< Get containing list pointer. */

#define list_get_head_item_value(p_list)               ((((p_list)->end_item).next_ptr)->value)             /**< Get value of head item. */
#define list_get_head_item(p_list)                     (((p_list)->end_item).next_ptr)                      /**< Get head item pointer. */
#define list_get_owner_of_head_item(p_list)            (list_item_get_owner(((p_list)->end_item).next_ptr)) /**< Get owner of head item. */
#define list_get_end_item(p_list)                      ((p_list)->end_item)                                 /**< Get list end sentinel item. */
#define list_get_num_item(p_list)                      (((p_list)->num_of_items))                           /**< Get number of real items in list. */
#define list_is_empty(p_list)                          (((p_list)->num_of_items == (uint8_t)0) ? OS_TRUE : OS_FALSE) /**< Check whether list is empty. */
