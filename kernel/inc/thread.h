...
/** @brief Opaque handle to an internal thread control block. */
typedef struct thread_tcb *thread_handle_t;

/** @brief Static thread descriptor type used by @ref AKOS_THREAD_DEFINE. */
...
/**
 * @brief Runtime accounting snapshot for one thread slot.
 */
typedef struct
{
#if (AKOS_CFG_USE_SHELL != 0u)
  ...
  uint32_t run_ticks;                           /**< Accumulated runtime in ticks. */
